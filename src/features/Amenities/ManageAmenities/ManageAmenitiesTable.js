...
import './ManageAmenitiesTable.scss'
...
import { fetchAmenitiesList, updateTableState, fetchAmenitiesListTableColumnConfig } from './manageAmenitiesTableSlice';
import { fetchLookupOptionsSearch, deleteAmenities } from './ManageAmenitiesApi';

const STORE_ID = localStorage.getItem("storeId");

function ManageAmenitiesTable() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { t } = useTranslation();

  const userPermission = localStorage.getItem('roles');
  const columnsOriginalResponse = useSelector((state) => state.manageAmenitiesTableReducer.columnsOriginalResponse);
  const data = useSelector((state) => state.manageAmenitiesTableReducer.data);
  const loading = useSelector((state) => state.manageAmenitiesTableReducer.loading);
  const total = useSelector((state) => state.manageAmenitiesTableReducer.total);
  const page = useSelector((state) => state.manageAmenitiesTableReducer.page);
  const rowsPerPage = useSelector((state) => state.manageAmenitiesTableReducer.rowsPerPage);
  const sortBy = useSelector((state) => state.manageAmenitiesTableReducer.sortBy);
  const sortKey = useSelector((state) => state.manageAmenitiesTableReducer.sortKey);
  const appliedFilterList = useSelector((state) => state.manageAmenitiesTableReducer.appliedFilterList);
  ...
  const [showDeleteAmenitiesPopup, setShowDeleteAmenitiesPopup] = useState({
    show: false,
    data: null
  });
  ...
  const fetchTable = async () => {
    ...
    await dispatch(fetchAmenitiesList({ data: filterData, params: { perPage: rowsPerPage, page: page, sortBy, sortKey } }));
    clearFilterValues();
  }
  ...
  const handleTableButtonAction = async (e, action, tableMeta) => {
    ...
    if (index !== -1) {
      if (action === CRUD_ACTION.EDIT) {
        dispatch(updateTableState({ selectedAmenitiesData: selectedRow, flow: FLOW_TYPE.EDIT }));
        navigate('/create-amenities');
      } else if (action === CRUD_ACTION.DELETE) {
        const { amenitiesId } = selectedRow;
        setShowDeleteAmenitiesPopup((state) => {
          return {
            ...state,
            show: true,
            data: { amenitiesId }
            ...
            options: {
              ...
              customBodyRender: (value, tableMeta, updateValue) => {
                ...
                return (
                  ...
                  {userPermission !== null && userPermission.includes("AMENITIES:EDIT") &&
                     <>
              ...
              {userPermission !== null && userPermission.includes("AMENITIES:DELETE") &&
                    <>
                    <Tooltip title={"Delete Amenities"}>
  ...
  async function onPageLoad() {
    //Note:: Fetch manageAmenitiesTableConfig 
    dispatch(updateTableState({ appliedFilterList: [], page: 0, sortBy: "", sortKey: "", rowsPerPage: 10, total: 0 }));
    // if (columnsOriginalResponse.length === 0) {
    await dispatch(fetchAmenitiesListTableColumnConfig({ params: {} }));
    // }
  ...
  const closeModalHandler = (modalName) => {
    if (modalName === "showDeleteAmenitiesPopup") {
      setShowDeleteAmenitiesPopup((state => {
        return {
          ...state,
          show: false
  ...
  const deleteAmenitiesHandler = async () => {
    dispatch(updateTableState({ loading: true }));
    const { amenitiesId } = showDeleteAmenitiesPopup.data;
    const response = await deleteAmenities({ amenitiesId }, { isDeleted: true }, dispatch);
    if (response === "Success") {
      setShowDeleteAmenitiesPopup((state) => {
        return {
          ...state,
  ...
  return (
    <div className="manage-amenities-table">
      <AlertDialog
        title={t("Confirmation")}
        modalName="delete-amenities-modal"
        open={showDeleteAmenitiesPopup.show}
        hideCloseButton={true}
        closeModalHandler={() => { closeModalHandler('showDeleteAmenitiesPopup') }}
        submitModalHandler={deleteAmenitiesHandler}
      >
        <span>Are you sure you want to delete this amenities?</span>
      </AlertDialog>
      <MUIDataTable
        title={t("Amenities")}
        tableClassName={`manage-amenities-table compact`}
        columns={columns}
        data={data}
        options={options}
        loading={loading}
...
export default ManageAmenitiesTable
